...
export const itemTextById: Record<string, DefText> = {
  ...
  'v3-golden-kennel': { name: 'Untouchable Golden Kennel', description: 'Gain 14 Shield. While shielded, incoming Poison and Weak are blocked.' },
  ...
  'v5-shattered-tooth-gear': { name: 'Shattered Tooth Gear', description: 'Counts allied 1-slot gear triggers. Every 4 counts reset and deal direct damage.' },
  'v5-poison-blood-pump': { name: 'Poison Blood Pump', description: 'Converts enemy Poison stacks into healing, up to 4 tiers.' },
  'v5-biteback-shield': { name: 'Biteback Shield', description: 'Gains Shield, then deals direct damage based on current Shield up to a cap.' },
  'v5-barkproof-earmuffs': { name: 'Barkproof Earmuffs', description: 'After repeated enemy gear triggers, disables their next 1-slot gear trigger once.' },
  'v5-offbeat-metronome': { name: 'Offbeat Metronome', description: 'Suppresses the enemy next Multi extra triggers, with stronger qualities refreshing after cooldown.' },
  'v5-bitter-kibble': { name: 'Bitter Kibble', description: 'Cleanses your Poison stacks and converts each cleansed stack into Shield.' },
  'v5-thornbreaker-chew': { name: 'Thornbreaker Chew', description: 'Removes enemy Thorns before hitting and deals extra damage to Shield.' },
...
export const ruleTermTextByTerm: Record<string, RuleTermText> = {
  ...
  '护盾': { term: 'Shield', description: 'Temporary protection that absorbs normal damage first. It cannot be stolen, but some buff-purge effects remove it in chunks of 8 Shield per stack.', note: 'While Shield exists, some gear blocks incoming Poison and Weak.' },
  ...
  '虚弱': { term: 'Weak', description: 'Negative effect. The target next attack deals 50% less damage, then consumes 1 stack.', note: 'Some shield-immunity gear blocks incoming Weak.' },
