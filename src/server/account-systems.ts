...
function eventCount(kind: AccountEvent['kind']) {
  return (event: AccountEvent) => event.kind === kind ? 1 : 0
}

function battleWin(event: AccountEvent) {
  return event.kind === 'BATTLE_FINISHED' && event.winner ? 1 : 0
}

function battleMode(mode: string) {
  return (event: AccountEvent) => event.kind === 'BATTLE_FINISHED' && event.mode === mode ? 1 : 0
}

function dogBattleWin(dogType: DogType) {
  return (event: AccountEvent) => event.kind === 'BATTLE_FINISHED' && event.winner && event.dogType === dogType ? 1 : 0
}

function dogRun(dogType: DogType) {
  return (event: AccountEvent) => event.kind === 'RUN_CREATED' && event.dogType === dogType ? 1 : 0
}

function highRun(targetWins: number) {
  return (event: AccountEvent) => event.kind === 'BATTLE_FINISHED' && event.wins >= targetWins ? 1 : 0
}
...
export const ACHIEVEMENTS: AchievementDef[] = [
  ...
  { id: 'shiba-start', title: '柴犬出击', description: '使用柴犬开始 3 局。', category: '职业特色', target: 3, reward: 100, eventKinds: ['RUN_CREATED'], progress: dogRun('SHIBA') },
  { id: 'shiba-win', title: '小点快刀', description: '使用柴犬赢得 5 场战斗。', category: '职业特色', target: 5, reward: 140, eventKinds: ['BATTLE_FINISHED'], progress: dogBattleWin('SHIBA') },
  { id: 'shiba-high', title: '忍法十二胜', description: '使用柴犬达到 12 胜。', category: '职业特色', target: 1, reward: 260, eventKinds: ['BATTLE_FINISHED'], progress: (event) => event.kind === 'BATTLE_FINISHED' && event.dogType === 'SHIBA' && event.wins >= 12 ? 1 : 0 },
  { id: 'shiba-hidden', title: '影分身练习', description: '隐藏成就：使用柴犬完成高胜局。', category: '职业特色', target: 1, reward: 300, hidden: true, eventKinds: ['BATTLE_FINISHED'], progress: (event) => event.kind === 'BATTLE_FINISHED' && event.dogType === 'SHIBA' && event.wins >= 10 && event.losses <= 1 ? 1 : 0 },
  { id: 'samoyed-start', title: '萨摩耶登场', description: '使用萨摩耶开始 3 局。', category: '职业特色', target: 3, reward: 100, eventKinds: ['RUN_CREATED'], progress: dogRun('SAMOYED') },
  { id: 'samoyed-win', title: '大点雪线', description: '使用萨摩耶赢得 5 场战斗。', category: '职业特色', target: 5, reward: 140, eventKinds: ['BATTLE_FINISHED'], progress: dogBattleWin('SAMOYED') },
  { id: 'samoyed-high', title: '雪崩守门员', description: '使用萨摩耶达到 12 胜。', category: '职业特色', target: 1, reward: 260, eventKinds: ['BATTLE_FINISHED'], progress: (event) => event.kind === 'BATTLE_FINISHED' && event.dogType === 'SAMOYED' && event.wins >= 12 ? 1 : 0 },
  { id: 'samoyed-hidden', title: '绝对零度', description: '隐藏成就：使用萨摩耶完成高胜局。', category: '职业特色', target: 1, reward: 300, hidden: true, eventKinds: ['BATTLE_FINISHED'], progress: (event) => event.kind === 'BATTLE_FINISHED' && event.dogType === 'SAMOYED' && event.wins >= 10 && event.losses <= 1 ? 1 : 0 },
  { id: 'mutt-start', title: '土狗开跑', description: '使用土狗开始 3 局。', category: '职业特色', target: 3, reward: 100, eventKinds: ['RUN_CREATED'], progress: dogRun('MUTT') },
  { id: 'mutt-win', title: '额外投掷', description: '使用土狗赢得 5 场战斗。', category: '职业特色', target: 5, reward: 140, eventKinds: ['BATTLE_FINISHED'], progress: dogBattleWin('MUTT') },
  { id: 'mutt-high', title: '追车冠军', description: '使用土狗达到 12 胜。', category: '职业特色', target: 1, reward: 260, eventKinds: ['BATTLE_FINISHED'], progress: (event) => event.kind === 'BATTLE_FINISHED' && event.dogType === 'MUTT' && event.wins >= 12 ? 1 : 0 },
  { id: 'mutt-hidden', title: '咬尾循环', description: '隐藏成就：使用土狗完成高胜局。', category: '职业特色', target: 1, reward: 300, hidden: true, eventKinds: ['BATTLE_FINISHED'], progress: (event) => event.kind === 'BATTLE_FINISHED' && event.dogType === 'MUTT' && event.wins >= 10 && event.losses <= 1 ? 1 : 0 },
  { id: 'bully-start', title: '恶霸入场', description: '使用恶霸开始 3 局。', category: '职业特色', target: 3, reward: 100, eventKinds: ['RUN_CREATED'], progress: dogRun('BULLY') },
  { id: 'bully-win', title: '大件压制', description: '使用恶霸赢得 5 场战斗。', category: '职业特色', target: 5, reward: 140, eventKinds: ['BATTLE_FINISHED'], progress: dogBattleWin('BULLY') },
  { id: 'bully-high', title: '巨兽推进', description: '使用恶霸达到 12 胜。', category: '职业特色', target: 1, reward: 260, eventKinds: ['BATTLE_FINISHED'], progress: (event) => event.kind === 'BATTLE_FINISHED' && event.dogType === 'BULLY' && event.wins >= 12 ? 1 : 0 },
  { id: 'bully-large-master', title: '金库守卫', description: '使用恶霸赢得 10 场战斗。', category: '职业特色', target: 10, reward: 220, eventKinds: ['BATTLE_FINISHED'], progress: dogBattleWin('BULLY') },
  { id: 'emperor-start', title: '皇帝巡游', description: '使用狗皇帝开始 3 局。', category: '职业特色', target: 3, reward: 100, eventKinds: ['RUN_CREATED'], progress: dogRun('EMPEROR') },
  { id: 'emperor-win', title: '天命落点', description: '使用狗皇帝赢得 5 场战斗。', category: '职业特色', target: 5, reward: 140, eventKinds: ['BATTLE_FINISHED'], progress: dogBattleWin('EMPEROR') },
  { id: 'emperor-high', title: '犬王圣旨', description: '使用狗皇帝达到 12 胜。', category: '职业特色', target: 1, reward: 260, eventKinds: ['BATTLE_FINISHED'], progress: (event) => event.kind === 'BATTLE_FINISHED' && event.dogType === 'EMPEROR' && event.wins >= 12 ? 1 : 0 },
  { id: 'emperor-hidden', title: '亡国之君', description: '隐藏成就：使用狗皇帝完成高胜局。', category: '职业特色', target: 1, reward: 300, hidden: true, eventKinds: ['BATTLE_FINISHED'], progress: (event) => event.kind === 'BATTLE_FINISHED' && event.dogType === 'EMPEROR' && event.wins >= 10 && event.losses <= 1 ? 1 : 0 },
...
export function progressAchievements(progress: AchievementProgress[], event: AccountEvent, now = nowIso()): AchievementProgress[] {
  ...
  for (const def of ACHIEVEMENTS) {
    ...
    byId.set(def.id, {
      ...
      completedAt: nextProgress >= def.target ? now : null,
    })
  }
  return [...byId.values()]
}
...
export function claimAchievementReward(wallet: CurrencyWallet, progress: AchievementProgress[], achievementId: string, now = nowIso()) {
  ...
  return {
    wallet: { ...wallet, balance: wallet.balance + def.reward },
    progress: progress.map((item) => item.achievementId === achievementId ? { ...item, claimedAt: now } : item),
    amount: def.reward,
  }
}
...
function pickTask(slot: DailyTaskSlot, seed: string, salt: string) {
  const pool = DAILY_TASK_DEFS.filter((task) => task.slot === slot)
  return pool[seededIndex(seed, pool.length, salt)]
}
...
function dailyTasksFromSeed(dateKey: string, userId: string, salt = 'initial'): DailyTaskProgress[] {
  ...
  return (['PARTICIPATION', 'BUILD', 'BATTLE'] as const).map((slot) => {
    const task = pickTask(slot, seed, slot)
    return {
      taskId: task.id,
      slot,
      progress: 0,
...
export function createInitialDailyTasks(userId: string, dateKey: string): DailyTaskState {
  return {
    dateKey,
    refreshUsed: false,
    tasks: dailyTasksFromSeed(dateKey, userId),
  }
}
...
export function progressDailyTasks(state: DailyTaskState, event: AccountEvent): DailyTaskState {
  return {
    ...
    tasks: state.tasks.map((task) => {
      ...
      if (!def) return task
      return { ...task, progress: clampProgress(task.progress + def.progress(event), task.target) }
    }),
  }
}
...
export function claimDailyTaskReward(wallet: CurrencyWallet, state: DailyTaskState, taskId: string, now = nowIso()) {
  ...
  return {
    wallet: { ...wallet, balance: wallet.balance + task.reward },
    state: { ...state, tasks: state.tasks.map((entry) => entry.taskId === taskId ? { ...entry, claimedAt: now } : entry) },
    amount: task.reward,
  }
}
...
export function grantMatchCurrency(wallet: CurrencyWallet, result: { mode: 'CASUAL' | 'LADDER' | 'DOGFIGHT' | string; winner: boolean }) {
  ...
  return {
    wallet: {
      balance: wallet.balance + grant,
      dailyEarned: wallet.dailyEarned + grant,
    },
    amount: grant,
  }
}
...
export function purchaseCatalogItem(wallet: CurrencyWallet, inventory: CosmeticInventoryEntry[], catalogItemId: string, now = nowIso()) {
  ...
  return {
    wallet: { ...wallet, balance: wallet.balance - item.price },
    inventory: [...inventory, { catalogItemId, acquiredAt: now }],
    item,
  }
}
...
export function equipCosmetic(inventory: CosmeticInventoryEntry[], equipped: EquippedCosmetic[], catalogItemId: string) {
  ...
  return [
    ...equipped.filter((entry) => entry.slot !== item.type),
    { slot: item.type, catalogItemId },
  ]
}
