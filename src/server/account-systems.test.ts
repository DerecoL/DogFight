...
describe('account shop and achievements domain', () => {
  it('defines the first release achievement shape', () => {
    ...
    expect(ACHIEVEMENTS.filter((entry) => entry.category === '职业特色')).toHaveLength(24)
    expect(ACHIEVEMENTS.filter((entry) => entry.hidden)).toHaveLength(6)
    expect(ACHIEVEMENTS.map((entry) => entry.id)).toContain('frog-start')
  })
