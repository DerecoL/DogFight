...
describe('multi equipment', () => {
  it('fully triggers a multi item once per multi count', () => {
    ...
    expect(discEvents.map((event) => event.multiIndex)).toEqual([1, 2, 3])
    expect(discEvents.map((event) => event.multiTotal)).toEqual([3, 3, 3])
  })
  ...
  it('lets lotus sea increase adjacent multi count up to the cap', () => {
    ...
    expect(discEvents.every((event) => event.multiTotal === 5)).toBe(true)
  })
