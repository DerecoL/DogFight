...
describe('selection screen structure', () => {
  ...
  it('routes remaining low-frequency controls through handdrawn primitives', () => {
    ...
    expect(app).toContain('className="history-open-action account-panel-button"')
  ...
  it('keeps active run stats out of the mode lobby top banner', () => {
    const lobbyBranchStart = app.indexOf("if (appScreen === 'LOBBY')")
    const lobbyBranchEnd = app.indexOf("if (appScreen === 'SHOP')", lobbyBranchStart)
    const lobbyBranch = app.slice(lobbyBranchStart, lobbyBranchEnd)

    expect(lobbyBranch).toContain('<ModeLobby run={run}')
  ...
  it('wires account shop, achievements, personal settings, daily tasks, and cosmetics into the lobby shell', () => {
    expect(app).toContain("type AppScreen = 'LOBBY' | 'CASUAL' | 'LADDER' | 'DOGFIGHT' | 'PEAK' | 'SHOP' | 'ACHIEVEMENTS' | 'SETTINGS'")
    ...
    expect(app).toContain("api<AchievementsResponse>(`/achievements/${achievementId}/claim`")
    expect(app).toContain("api<DailyTasksResponse>(`/daily-tasks/${taskId}/claim`")
    ...
    expect(app).toContain('function AccountSettingsScreen')
    expect(app).toContain('function CosmeticBadge')
    expect(app).toContain("onEnterSettings={() => setAppScreen('SETTINGS')}")
    expect(app).toContain('className="account-panel-actions"')
    expect(app).toContain("cosmeticGroups.map((group)")
    expect(app).toContain('account-currency-pill')
    expect(app).toContain('shop-section-grid')
    expect(app).toContain("entry.hidden ? 'hidden' : ''")
    expect(app).toContain('daily-task-row')
    expect(css).not.toContain('.account-hub-actions')
    expect(css).toContain('.account-panel-actions')
    expect(css).toContain('.account-panel-button')
  ...
  it('applies dogfight run action results before background room refresh', () => {
    expect(app).toContain("from './shop-optimistic'")
    expect(app).toContain('onRoomChange(mergeDogfightRunPreview(room, data.run))')
    expect(app).toContain('void refreshRoom().catch(() => undefined)')
  ...
  it('wires peak arena to apex APIs and keeps the lobby button in the top banner', () => {
    ...
    expect(app).toContain('防守连胜')
    expect(app).not.toContain('reports.overall.challengeWins')
    expect(app).toContain('candidates: current?.candidates.filter((run) => run.id !== runId) ?? []')
    expect(app).toContain('selectedApexEntry')
    expect(app).toContain('className="apex-config-overlay"')
    expect(app).toContain('className="apex-config-sheet"')
  ...
  it('exposes stable hooks for the handdrawn polish pass', () => {
    ...
    expect(app).toContain('<RelicRail relics={snapshot.relics ?? []} compact />')
  ...
  it('exposes stable hooks for the battle vfx causality pass', () => {
    ...
    expect(app).toContain("'data-vfx-anchor': anchor")
    expect(app).toContain("'data-vfx-side': side")
    expect(app).toContain("'data-vfx-item-id': itemId")
    expect(app).toContain("battleVfxAnchorAttrs('equipment-row', owner, item.id)")
  ...
  it('keeps overlapping battle vfx instances alive until their own duration ends', () => {
    ...
    expect(app).not.toContain("activeFxInstances.filter((instance) => instance.presentation.kind === 'roll').map")
    expect(app).toContain("fx.kind === 'none' || fx.kind === 'roll'")
