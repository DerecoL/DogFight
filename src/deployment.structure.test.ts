...
describe('production deployment assets', () => {
  ...
  it('adds GitHub Actions deployment and server backup scripts', () => {
    ...
    expect(workflow).toContain('tar -xzf "$REMOTE_PACKAGE" -C "$TEMP_DEPLOY_PATH"')
    ...
    expect(workflow).toContain('BACKUP_REASON=predeploy sh deploy/backup-postgres.sh')
    expect(workflow).toContain('docker compose build --no-cache api caddy')
    expect(workflow).toContain('docker compose up -d --build')
    expect(workflow).toContain('docker compose up -d --build --force-recreate --no-deps caddy')
    ...
    expect(workflow).toContain("body.database !== 'ok'")
    expect(workflow).toContain('docker compose exec -T caddy cat /srv/index.html')
    expect(workflow).toContain('Caddy image does not match expected frontend assets')
    expect(workflow).toContain('https://www.torcharena.online')
    expect(workflow).toContain('Public frontend still has not switched to')
