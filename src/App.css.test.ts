...
describe('equipment layout scale', () => {
  ...
  it('keeps desktop battle playback focused on large equipment rails over empty stage space', () => {
    ...
    expect(cssRule('.battle-item .item-icon')).toContain('width: 46px')
    expect(cssRule('.battle-item .item-icon')).toContain('height: 46px')
  ...
  it('keeps battle equipment card copy bounded while preserving full details for inspection', () => {
    expect(app).not.toContain('const battleNameScale')
    expect(app).toContain('className="battle-item-name"')
    expect(app).toContain('className="battle-item-copy"')
    expect(app).toContain('className="battle-item-icon-frame"')
    expect(app).toContain('className="battle-enchant-overlay"')
    expect(app).toContain('className="battle-card-effect item-effect"')
    expect(app).toContain('item.enchant ? ` · ${enchantmentText(item.enchant)}` : ""')
    expect(cssRule('.battle-item .quality-chip')).toContain('position: absolute')
    expect(cssRule('.battle-item .quality-chip')).toContain('top: 6px')
    expect(cssRule('.battle-item .quality-chip')).toContain('left: 6px')
    expect(cssRule('.battle-item-name')).toContain('white-space: nowrap')
    expect(cssRule('.battle-item-name')).toContain('font-size: 13px')
    expect(cssRule('.battle-item-name')).toContain('overflow: hidden')
    expect(cssRule('.battle-item-copy')).toContain('grid-template-rows: 18px 18px minmax(0, 1fr)')
    expect(cssRule('.battle-enchant-overlay')).toContain('grid-row: 1')
    ...
    expect(cssRule('.battle-item .battle-card-effect')).toContain('-webkit-line-clamp: 2')
  ...
  it('keeps battle equipment icon frames and text rows fixed', () => {
    ...
    expect(cssRule('.battle-item')).toContain('grid-template-rows: 72px minmax(0, 1fr)')
    expect(cssRule('.battle-item-icon-frame')).toContain('width: 62px')
    expect(cssRule('.battle-item-icon-frame')).toContain('height: 62px')
    expect(cssRule('.battle-item-copy')).toContain('grid-template-rows: 18px 18px minmax(0, 1fr)')
    expect(cssRule('.battle-item-copy')).toContain('align-self: end')
    expect(cssRule('.battle-item-name')).toContain('grid-row: 1')
    expect(cssRule('.battle-item-meta')).toContain('grid-row: 2')
    expect(cssRule('.battle-item .battle-card-effect')).toContain('grid-row: 3')
  })
