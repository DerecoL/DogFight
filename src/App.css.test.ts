...
describe('exploration map route board', () => {
  ...
  it('expands the route canvas, thickens routes, and increases node stagger', () => {
...
describe('equipment layout scale', () => {
  ...
  it('keeps the center dice as a three-dimensional handdrawn die after shared UI styles load', () => {
    expect(primitives).toContain('className="dynamic-dice-base"')
    expect(primitives).toContain('src="/assets/ui/battle-dice-base.webp"')
    expect(primitives).not.toContain('className="dynamic-dice-face top"')
    expect(primitives).not.toContain('className="dynamic-dice-face side"')
    expect(lastUiCssRule('.battle-dice.dynamic-dice')).toContain('perspective')
    expect(lastUiCssRule('.battle-dice.dynamic-dice')).toContain('--dice-size: 96px')
    expect(lastUiCssRule('.battle-dice.dynamic-dice .dynamic-dice-cube')).toContain('transform: rotate(-4deg)')
    expect(lastUiCssRule('.battle-dice.dynamic-dice .dynamic-dice-cube')).not.toContain('rotateX(58deg)')
    expect(lastUiCssRule('.dynamic-dice-base')).toContain('object-fit: contain')
    expect(lastUiCssRule('.battle-dice.dynamic-dice .dynamic-dice-face')).toContain('position: absolute')
    expect(lastUiCssRule('.battle-dice.dynamic-dice .dynamic-dice-face.top, .battle-dice.dynamic-dice .dynamic-dice-face.side')).toContain('display: none')
    expect(lastUiCssRule('.battle-dice.dynamic-dice .dynamic-dice-face.front')).toContain('transform: none')
    expect(lastUiCssRule('.battle-dice.dynamic-dice .dynamic-dice-face.front')).toContain('left: 18%')
    expect(lastUiCssRule('.battle-dice.dynamic-dice .dynamic-dice-face.front')).toContain('width: 48%')
    expect(lastUiCssRule('.battle-dice.dynamic-dice .dynamic-dice-face')).toContain('background: transparent')
    expect(lastUiCssRule('.battle-dice.dynamic-dice .dynamic-dice-pips i')).toContain('opacity: 0')
    expect(lastUiCssRule('.battle-dice.dynamic-dice .dynamic-dice-pips i')).toContain('width: 11px')
    expect(lastUiCssRule('.battle-dice.dynamic-dice .dynamic-dice-value')).toContain('position: absolute')
    expect(lastUiCssRule('.battle-dice.dynamic-dice .dynamic-dice-value')).toContain('right: 13px')
  ...
  it('keeps desktop battle playback focused on large equipment rails over empty stage space', () => {
    ...
    expect(cssRule('.battle-equipment-row')).toContain('grid-template-columns: minmax(0, 1fr) 168px')
    expect(cssRule('.battle-equipment-row')).toContain('align-items: stretch')
    expect(cssRule('.battle-equipment-row .relic-rail')).toContain('width: 168px')
    expect(cssRule('.battle-equipment-row .relic-slot-grid')).toContain('grid-template-columns: repeat(3, 46px)')
    expect(cssRule('.battle-equipment-row .relic-slot-grid')).toContain('grid-template-rows: repeat(2, 46px)')
    expect(cssRule('.battle-item')).toContain('min-height: 142px')
    expect(cssRule('.battle-item-icon-frame')).toContain('width: 62px')
    expect(cssRule('.battle-item-icon-frame')).toContain('height: 62px')
    expect(cssRule('.battle-item .item-icon')).toContain('width: 56px')
    expect(cssRule('.battle-item .item-icon')).toContain('height: 56px')
  ...
  it('keeps battle equipment cards simplified while preserving full details for inspection', () => {
    expect(app).not.toContain('const battleNameScale')
    expect(app).toContain('className="battle-item-name"')
    expect(app).toContain('className="battle-item-info"')
    expect(app).toContain('className="battle-item-dice"')
    expect(app).toContain('className="battle-item-icon-frame"')
    expect(app).toContain('className="battle-enchant-overlay"')
    expect(app).toContain('className="battle-item-growth-number"')
    expect(app).toContain('const growthNumber = growthDamageValueForBattleItem')
    expect(app).not.toContain('<small className="battle-card-effect item-effect">{itemEffect}</small>')
    expect(app).toContain('item.enchant ? ` · ${enchantmentText(item.enchant)}` : ""')
    expect(cssRule('.battle-item')).toContain('grid-template-rows: 64px 21px 22px')
    expect(cssRule('.battle-item-name')).toContain('white-space: nowrap')
    expect(cssRule('.battle-item-name')).toContain('font-size: var(--battle-name-size, 13px)')
    expect(cssRule('.battle-item-name')).toContain('overflow: hidden')
    expect(cssRule('.battle-item-info')).toContain('grid-template-columns: minmax(0, 1fr) auto auto')
    expect(cssRule('.battle-item-info')).toContain('grid-row: 3')
    expect(cssRule('.battle-enchant-overlay')).toContain('white-space: nowrap')
    expect(cssRule('.battle-enchant-overlay')).toContain('text-overflow: ellipsis')
    expect(lastCssRule('.battle-item-growth-number')).toContain('font-variant-numeric: tabular-nums')
  })

  it('keeps the animated battle dice cube and avoids duplicate roll result bubbles', () => {
    ...
    expect(cssRule('.battle-dice.dynamic-dice .dynamic-dice-cube')).toContain('transform: rotate(-4deg)')
    expect(cssRule('.dynamic-dice-base')).toContain('object-fit: contain')
  ...
  it('renders the center battle dice with the compressed bitmap base instead of detached CSS faces', () => {
    expect(existsSync(battleDiceBaseUrl)).toBe(true)
    expect(statSync(battleDiceBaseUrl).size).toBeLessThanOrEqual(15 * 1024)
    ...
    expect(cssRule('.dynamic-dice-base')).toContain('width: 100%')
    expect(cssRule('.dynamic-dice-base')).toContain('height: 100%')
    expect(cssRule('.dynamic-dice-face.front')).toContain('width: 48%')
    expect(cssRule('.dynamic-dice-face.front')).toContain('transform: none')
    expect(primitives).not.toContain('className="dynamic-dice-face top"')
    expect(primitives).not.toContain('className="dynamic-dice-face side"')
    expect(cssRule('.dynamic-dice-pips i')).toContain('opacity: 0')
    expect(css).toContain('42% { transform: rotate(10deg) translateY(-15px) scale(1.18); }')
  ...
  it('keeps battle equipment icon frames and text rows fixed', () => {
    ...
    expect(cssRule('.battle-item .item-icon')).toContain('width: 56px')
    expect(cssRule('.battle-item .item-icon')).toContain('height: 56px')
    expect(cssRule('.battle-item-name')).toContain('grid-row: 2')
    expect(cssRule('.battle-item-info')).toContain('grid-row: 3')
    expect(cssRule('.battle-item-info')).toContain('min-height: 22px')
  })
