...
func _run() -> void:
	...
	for part in ["天梯排行榜", "当前段位", "18 局 · 12胜 6负", "大师", "420分", "420 分 / 500 晋级犬王", "犬王积分榜", "进入犬王后参与排名", "还没有犬王，先冲上大师 500 分。"]:
		if not text.contains(str(part)):
			_fail("Ladder leaderboard label missing: %s" % str(part))
			return
	for raw in ["MASTER", "DOG_KING"]:
		if text.contains(str(raw)):
			_fail("Ladder leaderboard leaked raw tier: %s" % str(raw))
			return
	run_screen.set("leaderboard_data", {
		"playerRank": 2,
		"playerProfile": {
	...
	for part in ["你的犬王排名：第 2 名", "#1", "榜首柴", "880", "#2", "我的狗", "720", "720 分 · 犬王积分"]:
		if not text.contains(str(part)):
