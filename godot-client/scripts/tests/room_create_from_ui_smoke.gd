...
func _run() -> void:
	...
	if active_room.is_empty() or str(active_room.get("id", "")).is_empty():
		_fail("Create-room action should populate active_room")
		return
	if str(legacy.get("current_tab")) != "房间":
		_fail("Create-room action should keep the room tab visible")
		return
	if main.get("run_store").has_run():
		_fail("Create-room action must not create a casual or ladder run")
		return
	var text := _collect_text(legacy)
	for part in ["当前房间", "等待中", "离开房间", "开始房间", "房主"]:
		if not text.contains(part):
			_fail("Created room UI missing control: %s" % part)
			return
	if main.get_node_or_null("ScreenRoot/DogfightRoomDetailScreen").visible:
		_fail("Create-room action must not show the old standalone room detail screen")
		return
	if legacy.find_child("PlaceholderPanel", true, false) != null:
		_fail("Create-room action must not show placeholder content")
		return
...
func _new_logged_in_main() -> Node:
	...
	if account_input == null or password_input == null:
		...
		return null
	account_input.text = "godot-room-create-%d-%d" % [int(Time.get_unix_time_from_system()), Time.get_ticks_usec()]
