...
func _run() -> void:
	...
	var title_default := _find_by_name(run_screen, "CosmeticDefaultAction_TITLE") as Button
	if title_default == null or title_default.disabled:
		_fail("Title default action should be enabled when a title is equipped")
		return
	var avatar_default := _find_by_name(run_screen, "CosmeticDefaultAction_AVATAR") as Button
	if avatar_default == null or not avatar_default.disabled:
		...
		return
	var title_action := _find_by_name(run_screen, "CosmeticAction_title-paper-crown") as Button
	if title_action == null or not title_action.disabled:
		_fail("Equipped title action should be disabled")
		return
	var avatar_action := _find_by_name(run_screen, "CosmeticAction_avatar-crown") as Button
