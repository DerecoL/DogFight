...
func _run() -> void:
	...
	_assert_label_text(screen, "SeasonHistoryTitle", "赛季历史")
	_assert_label_text(screen, "SeasonHistorySubtitle", "2 个已结束赛季")
	_assert_history_card(screen, "season-1-summary", "第一赛季", "天梯 大师 · 520 分 · 犬王第 6 名", "巅峰未入榜", false)
	_assert_history_card(screen, "season-0-summary", "测试赛季", "天梯 黄金 · 80 分", "巅峰第 12 名 · 10胜2败", true)

	screen.call("set_payload", _season_payload([]))
	await process_frame
	_assert_label_text(screen, "SeasonHistoryTitle", "赛季历史")
	_assert_label_text(screen, "SeasonHistorySubtitle", "赛季结束后会保存在这里")
	_assert_label_text(screen, "SeasonHistoryEmpty", "暂无赛季历史")
