...
func _run() -> void:
	...
	if not source.contains("func _guarded_fetch_into"):
		...
		return
	var body := _function_body(source, "func _refresh_cosmetics")
	if body.is_empty():
		_fail("RunScreen missing _refresh_cosmetics")
		return
	var raw_check_body := body.replace("_guarded_fetch_into(", "")
	if raw_check_body.contains("_fetch_into("):
		_fail("_refresh_cosmetics must not call _fetch_into directly without action guard")
		return
	if not body.contains("_guarded_fetch_into("):
		_fail("_refresh_cosmetics must use guarded fetch")
		return
	print("Godot cosmetic refresh guard smoke passed")
	quit(0)
