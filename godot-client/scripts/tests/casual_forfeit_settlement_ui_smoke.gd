...
func _run() -> void:
	...
	if not await _start_casual_run_from_ui(main, legacy):
		return
	...
	if not await _start_and_finish_battle(main, legacy, router, run_id):
		return
	if not await _forfeit_and_assert_settlement(main, legacy, run_id):
		return
...
func _start_casual_run_from_ui(main: Node, legacy: Node) -> bool:
...
func _advance_to_match_phase(main: Node, legacy: Node, run_id: String) -> bool:
	for _step in range(16):
		...
		if phase == "MAP":
			...
			if not await _click_action_and_wait(legacy, "进入 ", "/runs/%s/choice/select" % run_id):
				_fail("CHOICE phase should expose a visible shop choice")
				return false
		elif phase == "RELIC_CHOICE":
			if not await _click_modal_action_and_wait(main, legacy, "查看遗物", "选择遗物", "/runs/%s/relic/select" % run_id):
				return false
		elif phase == "UPGRADE_CHOICE":
			if not await _click_action_and_wait(legacy, "跳过升级", "/runs/%s/upgrade/skip" % run_id):
				_fail("UPGRADE_CHOICE should expose skip action")
				return false
		elif phase == "POTION_CHOICE":
			if not await _select_first_item_for_target(main, legacy):
				return false
			if not await _click_modal_action_and_wait(main, legacy, "查看药水", "药水给选中装备", "/runs/%s/potion/select" % run_id):
				return false
		else:
			_fail("Settlement smoke reached unsupported pre-battle phase: %s" % phase)
			return false
		if not await _wait_for_idle(legacy):
			_fail("Run action should finish refreshing")
			return false
		_assert_playable_shell(main, "advancing casual run")
...
func _advance_map_phase(main: Node, legacy: Node, run_id: String) -> bool:
	...
	if not _dict(map_state, "pendingReward").is_empty():
		if await _click_action_and_wait(legacy, "领取怪物奖励", "/runs/%s/map/monster-reward/claim" % run_id):
			return true
		if await _click_action_and_wait(legacy, "跳过怪物奖励", "/runs/%s/map/monster-reward/skip" % run_id):
			return true
		_fail("Pending monster reward should expose claim or skip action")
		return false
...
func _forfeit_and_assert_settlement(main: Node, legacy: Node, run_id: String) -> bool:
	var forfeit_button = await _wait_for_button_containing(legacy, "放弃并结算")
	if forfeit_button == null:
		_fail("Active run should expose visible forfeit settlement action")
		return false
	forfeit_button.pressed.emit()
	var modal_layer = main.get_node_or_null("OverlayRoot/ModalLayer")
	if modal_layer == null:
		_fail("Forfeit confirmation should use ModalLayer")
		return false
	var confirm_button = await _wait_for_button_containing(modal_layer, "确认放弃并结算")
	if confirm_button == null:
		_fail("Forfeit modal should expose confirmation action")
		return false
	seen_paths.clear()
	confirm_button.pressed.emit()
	if not await _wait_for_path("/runs/%s/settle" % run_id):
		_fail("Forfeit confirmation should POST run settle")
		return false
	if not await _wait_for_idle(legacy):
		_fail("Forfeit settlement action should finish refreshing")
		return false
	for _frame in range(240):
		if _current_run_phase(main) == "COMPLETE" and _collect_text(legacy).contains("跑局结算"):
			_assert_playable_shell(main, "forfeit settlement")
			return true
		await process_frame
	_fail("Forfeit settlement should render the visible run settlement summary")
	return false

func _click_modal_action_and_wait(main: Node, legacy: Node, open_text: String, action_text: String, path: String) -> bool:
	var open_button = await _wait_for_button_containing(legacy, open_text)
	if open_button == null:
		_fail("Missing reward detail button: %s" % open_text)
		return false
	open_button.pressed.emit()
	var modal_layer = main.get_node_or_null("OverlayRoot/ModalLayer")
	if modal_layer == null:
		_fail("ModalLayer should exist for reward action")
		return false
	var action_button = await _wait_for_button_containing(modal_layer, action_text)
	if action_button == null:
		_fail("Missing modal reward action: %s" % action_text)
		return false
	seen_paths.clear()
	action_button.pressed.emit()
	if not await _wait_for_path(path):
		_fail("Modal action should POST %s" % path)
		return false
	return true

func _select_first_item_for_target(main: Node, legacy: Node) -> bool:
	if str(legacy.get("selected_item_id")) != "":
		return true
	var run := _current_run(main)
	for item in _array(run, "items"):
		if item is Dictionary:
			...
			if not item_id.is_empty():
				legacy.set("selected_item_id", item_id)
				legacy.call("_render_current_tab")
				await process_frame
				return true
	_fail("Reward target phase should have at least one selectable item")
	return false
...
func _new_logged_in_main() -> Node:
	...
	if account_input == null or password_input == null:
		...
		return null
	account_input.text = "godot-casual-settle-%d-%d" % [int(Time.get_unix_time_from_system()), Time.get_ticks_usec()]
...
func _assert_playable_shell(main: Node, context: String) -> void:
